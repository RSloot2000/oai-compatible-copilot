...
/**
 * Editor WebviewPanel that hosts the full OAICopilot configuration UI.
 *
 * All business logic lives in {@link ConfigController}; this class only
 * manages the panel lifecycle and wires the webview to the shared
 * controller. The sidebar view (ConfigSidebarView) reuses the same
...
export class ConfigViewPanel {
	...
	private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, secrets: vscode.SecretStorage) {
		...
		this.panel.webview.onDidReceiveMessage(
			async (message) => {
				this.controller.handleMessage(this.panel.webview, message).catch((err) => {
		...
		void this.controller.sendInit(this.panel.webview);
	}
